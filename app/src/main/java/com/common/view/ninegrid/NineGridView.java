...
public class NineGridView extends ViewGroup {
    ...
    public static int singleImageSize = 250;              // 单张图片时的最大大小,单位dp
    public static float singleImageRatio = 1.0f;          // 单张图片的宽高比(宽/高)
...
//            width = gridWidth * columnCount + gridSpacing * (columnCount - 1) + getPaddingLeft() + getPaddingRight();
        ...
        if (imageViews.size() == 1) {
            imageViews.get(0).setScaleType(ImageView.ScaleType.FIT_START);
        } else {
    ...
    private void match1PicSize(int width) {
        if (mImageInfo.get(0).viewHeight != 0 && mImageInfo.get(0).viewWidth != 0) {//已计算过，直接展示
            gridWidth = mImageInfo.get(0).viewWidth;
            gridHeight = mImageInfo.get(0).viewHeight;
        } else {//未计算过
            if (mImageInfo.get(0).getHeight() != 0 && mImageInfo.get(0).getWidth() != 0) { //拉伸写法
                gridWidth = mImageInfo.get(0).getWidth();
                gridHeight = mImageInfo.get(0).getHeight();
                if (gridWidth > gridHeight) {//以宽为基准拉伸
                    gridWidth = width;
                    gridWidth = gridWidth - getPaddingLeft() - getPaddingRight();
                    //计算高度  图片高*控件宽/图片宽
//                    gridHeight = singleImageSize > gridWidth ? gridWidth : singleImageSize;
                    gridHeight = gridHeight * gridWidth / mImageInfo.get(0).getWidth();
                } else {//以高为基准拉伸
                    int height = ScreenUtils.dip2px(getContext(), 250);
                    gridHeight = height;
                    gridHeight = gridHeight - getPaddingTop() - getPaddingBottom();
                    //计算宽度  控件高*图片宽/图片高
//                    gridHeight = singleImageSize > gridWidth ? gridWidth : singleImageSize;
                    gridWidth = gridHeight * gridWidth / mImageInfo.get(0).getHeight();
                }
            } else {
                //原居中方格写法
                int totalWidth = width - getPaddingLeft() - getPaddingRight();
            ...
            mImageInfo.get(0).viewWidth = gridWidth;
            mImageInfo.get(0).viewHeight = gridHeight;
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (childrenCount == 1) {
            ...
            com.nostra13.universalimageloader.core.ImageLoader.getInstance().displayImage(mImageInfo.get(0).url, childrenView, Config.circleIconDefault);
          ...
          /*  if (mImageLoader != null) {
                ...
                com.nostra13.universalimageloader.core.ImageLoader.getInstance().displayImage(mImageInfo.get(i).thu_url, childrenView, Config.circleIconDefault);
